...
## 1. 트랜잭션 경계 분리를 통한 참여 응답 속도 95% 단축
...
## 3. Circuit Breaker와 보상 배치를 통한 외부 장애 격리 및 미지급 건 최종 복구
...
1. **실패 상태와 재시도 조건 분리:** 복구 가능성이 낮은 4xx는 `FAILED`, 네트워크 오류·429·5xx·타임아웃은 `CALL_FAILED`로 구분했습니다. Retry도 네트워크 오류·429·5xx에만 최초 호출 포함 최대 3회(재시도 2회) 적용했습니다.
...
## 4. 리틀의 법칙 용량 산정과 AWS DLT 부하 테스트를 통한 20 TPS 안정성 검증
...
## 5. Strategy·Composite 패턴을 통한 이벤트 유형·정책의 독립적 확장 구조 확보
