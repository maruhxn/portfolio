...
## 1. 7-상태 상태 머신과 Inbox 멱등 처리를 통한 비동기 결제 정합성 확보
...
## 2. Transactional Outbox와 SKIP LOCKED를 통한 크레딧 지급 유실 복구와 중복 방지

### 문제

1. **지급 의도 유실:** 계좌 출금 성공을 저장한 직후 서버가 중단되면 돈은 빠졌지만 크레딧 지급 작업은 남지 않을 수 있었습니다.
2. **실패 전파:** 출금은 이미 자금이 이동한 확정 사실이라 결제 성공 기록은 반드시 커밋되어야 하지만, 크레딧 지급은 월렛이 없는 마트처럼 실패할 수 있는 작업입니다. 둘을 한 트랜잭션에 묶으면 지급 실패가 결제 성공 기록까지 되돌려 '돈은 빠졌는데 결제 기록이 없는' 상태를 만듭니다.
3. **재처리 중복:** 여러 인스턴스가 같은 지급 건을 동시에 가져가면 동일 크레딧이 반복 지급될 수 있었습니다.

### 해결

1. **Transactional Outbox:** 출금 성공 트랜잭션에서 크레딧을 직접 지급하지 않고 `CmsSubscriptionCreditOutbox`에 지급 의도를 함께 저장했습니다. 결제 성공과 지급 의도가 하나의 커밋으로 남고, 최초 결제·갱신·백오피스 수동 확인이 모두 이 경로를 지나 멱등 키를 공유합니다.
2. **커밋 필수성 기준의 경계:** 반드시 커밋되어야 하는 결제 기록과, 실패해도 재시도하면 되는 크레딧 지급을 서로 다른 트랜잭션으로 나눴습니다. 지급은 같은 DB에 대한 쓰기이므로 `크레딧 지급 → 완료 기록`은 한 트랜잭션에서 커밋해, 지급됐지만 완료로 기록되지 않은 중간 상태가 생기지 않게 했습니다. 롤백되면 남지 않는 실패 사유만 별도 트랜잭션에 기록합니다.
3. **SKIP LOCKED 선점과 멱등성:** `SELECT ... FOR UPDATE SKIP LOCKED`로 Outbox 행을 잠가, 다른 인스턴스는 대기하지 않고 다음 건으로 넘어갑니다. 락이 처리 트랜잭션 내내 유지되므로 별도의 처리 상태 없이 동시 처리를 막습니다. 청구 주기별 원장 `transactionId` UNIQUE 제약은 이중 방어로 남겼습니다.

### 결과

1. **지급 의도 보존:** 출금 성공과 크레딧 지급 의도 사이의 영구 유실 경로를 제거했습니다.
2. **결제 기록 보호:** 크레딧 지급이 실패해도 결제 성공 기록은 커밋된 채로 남고, 실패한 건만 사유와 함께 다음 배치가 재시도합니다.
3. **동시 처리 차단:** 다중 인스턴스가 같은 건을 잡지 못하도록 행 락으로 막았고, 잠긴 행을 기다리지 않고 건너뛰는지 실제 DB 통합 테스트로 검증했습니다.

| 장애 시점 | 남는 상태 | 복구 방식 |
| --- | --- | --- |
| 출금 성공 처리 전 | 결제 미완료 | 출금 결과에 따라 재시도·실패 처리 |
| 출금 성공 후, 지급 전 | Outbox `PENDING` | 다음 배치가 처리 |
| 지급 처리 중 서버 중단 | 트랜잭션 롤백 → Outbox `PENDING` | 락 해제 후 다음 배치가 재처리 |
| 크레딧 지급 실패 | Outbox `FAILED` + 실패 사유 | 결제 기록은 유지, 다음 배치가 재시도 |

## 3. 구독·청구·결제 상태 분리와 3일 유예를 통한 결제 실패 시 서비스 연속성 확보
