...
## 1. 골든셋(150키워드·5,284판정) 구축을 통한 재현 가능한 품질 평가 기준 확보
...
## 2. 하이브리드 검색(bge-m3 + RRF + 리랭크)을 통한 NDCG@10 +22.4% 개선
...
## 3. 전체·증분 색인 분리와 Outbox 재처리를 통한 무중단 검색 데이터 동기화
...
## 4. Datadog 구간 계측을 통한 지연 원인 규명과 병목 해소(5,010ms→100ms)
...
## 5. 사용자별 엔진 고정 A/B 테스트를 통한 온라인 평가 기반 구축
